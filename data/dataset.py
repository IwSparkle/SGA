...
from util.tool import csr_to_user_dict_bytime, csr_to_user_dict, csr_to_user_dict_order, csr_to_item_dict_order
from .utils import check_md5
from util.logger import Logger
from util import randint_choice, tool
import numpy as np
...
class Dataset(object):
    ...
    def _load_data(self, config):
        ...
        if file_format == "UI":
            ...
            all_ratings = [1.0] * len(all_data["user"])
        else:
        ...
        #
        self.all_user_matrix = csr_matrix((all_ratings, (all_data["user"], all_data["item"])),
                                     shape=(self.num_users, self.num_items))

        self.item_matrix = csr_matrix((train_ratings, (train_data["item"], train_data["user"])),
                                      shape=(self.num_items, self.num_users))

        self.all_item_matrix = csr_matrix((all_ratings, (all_data["item"], all_data["user"])),
                                          shape=(self.num_items, self.num_users))
    ...
    def get_user_train_dict_order(self):
        user_order_dict = csr_to_user_dict_order(self.train_matrix)
        return user_order_dict

    def get_item_train_dict_order(self):
        item_order_dict = csr_to_item_dict_order(self.item_matrix)
    ...
    def get_user_degree(self):
        user_degree = {}
        for idx, value in enumerate(self.train_matrix):
            if len(value.indices):
                user_degree[idx] = len(value.indices)
        return user_degree

    def get_item_degree(self):
        item_degree = {}
        for idx, value in enumerate(self.item_matrix):
            if len(value.indices):
                item_degree[idx] = len(value.indices)
        return item_degree