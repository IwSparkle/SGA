...
class SGL(AbstractRecommender):
    def __init__(self, sess, dataset, conf):
        ...
        self.user_ratio = conf["user_ratio"]
        self.item_ratio = conf["item_ratio"]


        self.user_order = self.dataset.get_user_train_dict_order()


        self.key_users = list(self.user_order.keys())[:int(self.n_users * self.user_ratio)]
        ...
        self.drop_weight = self.degree_drop_weightV2()
        self.threshold = conf["threshold"]
        self.keep_index = self.select_keep_weight(threshold=self.threshold)

        self.key_items = list(set(self.dataset.get_item_train_dict_order().keys()))[:int(self.n_items * self.item_ratio)]
    ...
    def create_adj_mat(self, is_subgraph=False, aug_type=0):
        ...
        if is_subgraph and aug_type in [0, 1, 2] and self.ssl_ratio > 0:
            ...
            if aug_type in [1, 2]:
                drop_index = randint_choice(len(self.training_user), size=int(len(self.training_user) * self.ssl_ratio), replace=False,
                                            exclusion=self.keep_index)
                num1 = len(set(drop_index))
                num2 = len(set(self.training_user))
                num3 = len(self.training_user)
                all_index = [i for i in range(len(self.training_user))]
                keep_idx = list(set(all_index)-set(drop_index))
    ...
    def train_model(self):
        ...
        with open("log.txt", 'a+') as f:
            f.writelines("dataset: "+str(self.dataset_name)+"\n")
            f.writelines("best_result@epoch: "+str(self.best_epoch)+"\t")
            f.writelines(str(buf))
            f.write('\r\n')
    ...
    def degree_drop_weight(self):
        weight_list = np.zeros((self.n_users, self.n_items), dtype=np.float32)
        for user_idx, _ in enumerate(self.user_pos_train):
            for item_idx in self.user_pos_train[user_idx]:
                u_degree = np.float(self.user_degree[user_idx])
                i_degree = np.float(self.item_degree[item_idx])
                weight_list[user_idx][item_idx] = math.log((u_degree+i_degree)/2)
        # weight_list = np.mat(weight_list)
        weight_list = sp.csr_matrix(weight_list)
        weight_list.data = (weight_list.data - weight_list.data.min())/(weight_list.data.max() - weight_list.data.min())
        return weight_list

    def degree_drop_weightV2(self):
        weight_list = np.zeros((self.n_users, self.n_items), dtype=np.float32)
        dok_matrix = self.dataset.train_matrix.todok()
        for (user_idx, item_idx), value in dok_matrix.items():
            u_degree = np.float(self.user_degree[user_idx])
            i_degree = np.float(self.item_degree[item_idx])
            weight_list[user_idx][item_idx] = math.log((u_degree+i_degree)/2)
        weight_list = sp.csr_matrix(weight_list)
        weight_list.data = (weight_list.data - weight_list.data.min())/(weight_list.data.max() - weight_list.data.min())
        weight_list = weight_list.todok()
        return weight_list


    def select_keep_weight(self, threshold=0.7):

        keep_index = []
        num = 0
        for (user_idx, item_idx), value in self.drop_weight.items():
            if value >= threshold:
                keep_index.append(num)
            num += 1
        return keep_index

    def select_keep_weight_v2(self, threshold=0.7):
        keep_index = []
        num = 0
        for (user_idx, item_idx), value in self.drop_weight.items():
            if (user_idx in self.drop_user_idx) or (item_idx in self.drop_item_idx):
                num += 1
                continue
            else:
                if value >= threshold:
    ...
    def write2csv(self):
        with open("%s_user.csv" % (self.dataset_name), "w") as f:
            writer = csv.writer(f)
            for key, value in self.user_degree.items():
                writer.writerow([key,value])
        print("user_degree wrote!")
        f.close()

        with open("%s_item.csv" % (self.dataset_name), "w") as f:
            writer = csv.writer(f)
            for key, value in self.item_degree.items():
                writer.writerow([key, value])
        print("item_degree wrote!")
        f.close()
