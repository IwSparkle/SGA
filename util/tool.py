...
def csr_to_user_dict_order(matrix):
    '''
    sort users group by degree
    '''
    ...
    for idx, value in enumerate(matrix):
        if len(value.indices):
            len_dict[idx] = len(value.indices)
    # order_dict = sorted(len_dict.items(), key=lambda x:x[1], reverse=True)
    user_order_dict = {k:v for k, v in sorted(len_dict.items(), key=lambda item: item[1], reverse=True)}
    return user_order_dict


def csr_to_item_dict_order(item_matrix):
    """
    sort items group by degree
    """
    len_dict = {}
    for idx, value in enumerate(item_matrix):
        if len(value.indices):
            len_dict[idx] = len(value.indices)
    item_order_dict = {k:v for k, v in sorted(len_dict.items(), key=lambda item: item[1], reverse=True)}
